...
class RoutingConfig(BaseModel):
    ...
    def get_agent_name(self, agent_id: str) -> str:
        ...
        if agent_id in self.execution_agents:
            return self.execution_agents[agent_id].name
        return agent_id  # Fallback to ID

    def get_all_topics(self) -> list[str]:
        """Get all configured topics.

        Returns list of all topics from agents and overrides.
        """
        topics: set[str] = set()
        for agent in self.agents.values():
            topics.update(agent.topics)
        topics.update(self.overrides.keys())
        return sorted(topics)

    def is_known_topic(self, topic: str) -> bool:
        """Check if a topic is configured.

        Returns True if topic is in any agent's topics or in overrides.
        """
        # Check overrides
        if topic in self.overrides:
            return True
