...
session management, routing configuration, escalation, and Q&A logging.
"""
...
class MessageRole(str, Enum):
    """Role of message sender in a session (T016)."""

    USER = "user"  # Question from calling agent
    ASSISTANT = "assistant"  # Answer from expert agent
    HUMAN = "human"  # Feedback from human during escalation


class SessionStatus(str, Enum):
    """Status of a conversation session (T017)."""
...
class ResponseStatus(str, Enum):
    """Status of a hub response (T019)."""

    RESOLVED = "resolved"  # Answer ready, no escalation needed
    PENDING_HUMAN = "pending_human"  # Escalated, waiting for human
    NEEDS_REROUTE = "needs_reroute"  # Human added context, need to re-ask
...
# =============================================================================
# Session Models (T014-T015)
# =============================================================================


class Message(BaseModel):
    """A single message in a session conversation (T015).

    Messages track the back-and-forth dialogue in a session,
    including questions, answers, and human feedback.
    """
    ...
    content: Annotated[
        str,
        Field(min_length=1, description="Message content"),
    ]
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    metadata: dict[str, Any] | None = Field(
        default=None,
        description="Additional context (e.g., confidence, escalation_id)",
    )


class Session(BaseModel):
    """Represents a conversation with an expert agent (T014).

    Sessions maintain conversation history for multi-turn interactions
    and preserve context for human feedback loops.
    """

    model_config = ConfigDict(frozen=False)  # Messages can be added

    id: str = Field(
        default_factory=lambda: str(uuid.uuid4()),
        description="Unique session identifier (UUID)",
    )
    agent_id: str = Field(description="Agent role (architect, product, qa)")
    feature_id: str = Field(default="", description="Feature this session relates to")
...
# =============================================================================
# Hub Response Models (T018-T020)
# =============================================================================


class HubResponse(BaseModel):
    """Response from ask_expert (T018).

    Contains the answer, confidence, session_id for follow-ups,
    and status indicating whether the answer is resolved or pending.
    """

    model_config = ConfigDict(frozen=True)

    answer: str = Field(default="", description="The expert's answer")
    rationale: str = Field(default="", description="Why this answer")
    confidence: Annotated[
        int,
        Field(ge=0, le=100, description="Confidence score (0-100)"),
    ]
    uncertainty_reasons: list[str] = Field(
        default_factory=list,
        description="Reasons for uncertainty if not 100%",
    )
    session_id: str = Field(description="Session ID for follow-ups")
    status: ResponseStatus = Field(description="resolved, pending_human, or needs_reroute")
    escalation_id: str | None = Field(
        default=None,
        description="Escalation ID if status is pending_human",
    )


class EscalationStatus(BaseModel):
    """Status of a pending human escalation (T020).

    Used by check_escalation to report current state and resolution details.
    """

    model_config = ConfigDict(frozen=True)

    escalation_id: str = Field(description="Unique escalation ID")
    status: str = Field(description="pending, resolved, or needs_reroute")
    action: HumanAction | None = Field(
        default=None,
        description="What human did (set when resolved)",
    )
    corrected_answer: str | None = Field(
        default=None,
        description="Human's corrected answer if action is CORRECT",
    )
    additional_context: str | None = Field(
        default=None,
        description="Added context if action is ADD_CONTEXT",
    )
...
class QALogEntry(BaseModel):
    ...
    session_id: str | None = Field(
        default=None,
        description="Session ID for conversation context (T067)",
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
